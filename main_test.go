...
func TestGetCustomersHandler(t *testing.T) {
	...
	handler := http.HandlerFunc(services.GetAllCustomers(filename))
...
func TestAddCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(services.AddCustomer(filename))
	handler.ServeHTTP(rr, req)
...
func TestDeleteCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(services.DeleteCustomer(filename))
	handler.ServeHTTP(rr, req)
...
func TestGetCustomerHandler(t *testing.T) {
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(services.GetOneCustomer((filename)))
	handler.ServeHTTP(rr, req)
